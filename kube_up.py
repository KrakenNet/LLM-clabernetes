...
def install_flannel():
    ...
    run_command("kubectl apply -f https://raw.githubusercontent.com/coreos/flannel/master/Documentation/kube-flannel.yml", "Deploying Flannel network plugin")

def install_containerd():
    """Installs and configures containerd for Kubernetes."""
    if check_installed("containerd"): 
        print("[INFO] Containerd is already installed. Skipping installation.")
        return
    run_command("sudo apt-get install -y containerd", "Installing Containerd")
    run_command("sudo mkdir -p /etc/containerd", "Creating Containerd config directory")
    run_command("containerd config default | sudo tee /etc/containerd/config.toml > /dev/null", "Generating default Containerd config")
    run_command("sudo systemctl restart containerd", "Restarting Containerd service")
    run_command("sudo systemctl enable containerd", "Enabling Containerd service")
...
def initialize_cluster():
    """Initializes the Kubernetes cluster using kubeadm with containerd as CRI."""
    if os.path.exists("/etc/kubernetes/admin.conf"):
        print("[SKIP] Kubernetes cluster already initialized.")
        return
    run_command("sudo kubeadm init --pod-network-cidr=10.244.0.0/16 --cri-socket unix:///run/containerd/containerd.sock --node-name=cluster-1", "Initializing Kubernetes cluster")
